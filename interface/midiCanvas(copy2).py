...
class MidiCanvas(FigureCanvas):
    ...
    def on_key_press(self, event):
        ...
        if event.key == "s":
            note_stream = self.get_music21_score() # gets midi score and saves as midi file

            mf = midi.translate.streamToMidiFile(note_stream)
            print(len(mf.tracks), len(mf.tracks[0].events))
            if self.filename:
                mf.open(self.filename[:-4] + '_midi.mid', 'wb')
            else:
                mf.open(self.filename + '_midi.mid', 'wb')
            mf.write()
            mf.close()
    ...
    def get_music21_score(self, note_duration=.25):
        ...
        if self.get_midi_lines():
            ...
            grid_lines = self.horizontal_snap_grid
            ...
            for ix, grid_line in enumerate(grid_lines):
                ...
                if grid_line in start_times:
                    midi_value = midi_values[start_times.index(grid_line)]
